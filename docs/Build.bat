...
javac %JAVAFX_ARGS% -sourcepath %PRAC_SRC% -cp %PRAC_BIN%;%PRAC_LIB% -d %PRAC_BIN% %PRAC_SRC%\csc2b\client\GUI.java
IF /I "%ERRORLEVEL%" NEQ "0" (
...
javadoc %JAVAFX_ARGS% -sourcepath %PRAC_SRC% -classpath %PRAC_BIN%;%PRAC_LIB% -use -version -author -d %PRAC_DOCS%\%PRAC_JDOC% -subpackages csc2b
IF /I "%ERRORLEVEL%" NEQ "0" (
...
java %JAVAFX_ARGS% -cp %PRAC_BIN%;%PRAC_LIB% GUI
IF /I "%ERRORLEVEL%" NEQ "0" (
